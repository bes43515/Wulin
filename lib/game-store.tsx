...
const starterAvatarItemIds = ["avatar-hair-black-topknot", "avatar-outfit-cloth", "avatar-weapon-wood", "avatar-outfit-sect", "avatar-weapon-iron", "avatar-headband-blue", "avatar-back-cape-plain", "avatar-aura-innerglow"];
const initialAvatarUserItems: UserItem[] = starterAvatarItemIds.map((item_id) => ({ user_id: "local-jianghu-player", item_id, quantity: 1, owned: true, acquired_at: new Date().toISOString() }));
const mergeStarterAvatarItems = (savedItems: UserItem[] | undefined) => {
  const saved = Array.isArray(savedItems) ? savedItems : [];
  const ownedIds = new Set(saved.filter((item) => item.owned).map((item) => item.item_id));
  return [...saved, ...initialAvatarUserItems.filter((item) => !ownedIds.has(item.item_id))];
};
